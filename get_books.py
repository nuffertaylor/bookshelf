...
def scrape_book_adjusted(book_id):
    url = 'https://www.goodreads.com/book/show/' + book_id
    source = urlopen(url)
    soup = bs4.BeautifulSoup(source, 'html.parser')
...
def scrape_books_from_shelf(userid, shelfname):
  book_ids = get_books_from_shelf(userid, shelfname)
  results = []
  for i, book_id in enumerate(book_ids):
    results.append(scrape_book_adjusted(book_id))
    print("got book data for " + results[i]["book_title"])
  return results